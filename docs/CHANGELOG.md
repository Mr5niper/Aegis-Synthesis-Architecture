...
- **UPX disabled in PyInstaller spec (`assistant_gui.spec`):** Changed `upx=True` to `upx=False` on both `EXE` and `COLLECT`. UPX frequently corrupts large native DLLs (torch, `llama_cpp`) and is a common cause of executables that crash on launch.
